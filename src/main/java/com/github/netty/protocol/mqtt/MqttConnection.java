...
public final class MqttConnection {
    ...
    private void processPubComp(MqttMessage msg) {
        ...
        final MqttSession session = sessionRegistry.retrieve(getClientId());
    ...
    private void processPubAck(MqttMessage msg) {
        final int messageID = ((MqttMessageIdVariableHeader) msg.variableHeader()).messageId();
        MqttSession session = sessionRegistry.retrieve(getClientId());
    ...
    private void setupInflightResender(Channel channel) {
        channel.pipeline()
                .addFirst("inflightResender", new MqttInflightResenderChannelHandler(5_000, TimeUnit.MILLISECONDS));
    }
    ...
    private boolean login(MqttConnectMessage msg, final String clientId) {
        ...
        if (msg.variableHeader().hasUserName()) {
            ...
            MqttUtil.userName(channel, login);
        } else if (!brokerConfig.isAllowAnonymous()) {
    ...
    public void handleConnectionLost() {
        ...
        MqttSession session = sessionRegistry.retrieve(clientID);
        if (session.hasWill()) {
    ...
    void processSubscribe(MqttSubscribeMessage msg) {
        ...
        postOffice.subscribeClientToTopics(msg, clientID, MqttUtil.userName(channel), this);
    }
    ...
    void processPublish(MqttPublishMessage msg) {
        ...
        switch (qos) {
            ...
            case EXACTLY_ONCE: {
                final int messageID = msg.variableHeader().packetId();
                final MqttSession session = sessionRegistry.retrieve(clientId);
...
//                msg.release();
    ...
    void sendPublish(MqttPublishMessage publishMsg) {
        ...
        if (LOG.isTraceEnabled()) {
            LOG.trace("Sending PUBLISH({}) message. MessageId={}, CId={}, topic={}, payload={}", qos, packetId,
                    clientId, topicName, MqttUtil.payload2Str(publishMsg.payload()));
        } else {
    ...
    String getClientId() {
        return MqttUtil.clientID(channel);
    }
